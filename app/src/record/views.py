...
def record():
    if request.method == "GET":
        ...
        ##視聴日(Movie.date)が一番最近のものから順にmovie_number(TMDbの映画ID),映画の視聴回数,一番最近見た日付,映画タイトル,ポスター画像,あらすじを取得。
...
def selected_movie(id):
    if request.method == "GET":
        ...
        #取得したidに一致するユーザーに紐づく映画を新しく見た日付順ですべて取得
        movies = Movie.query.where(Movie.user_id == current_user.id, Movie.movie_number == id).order_by(Movie.date.desc()).all()

        ##消さない
        #response = subtitles.search(tmdb_id=movie.movie_number, languages="en")

        lists = []

        #一時的
        response = {'index' : 1, 'start_seconds' : '0:00:50', 'content' : 'this is a pen.'}

        print(response)

        if response: #実際の実装時はresponse.data
            ...
            ##for item in srt:
                ...
                #正  results = re.findall(r'Subtitle\(index=(.*?[0-9]), start=datetime.timedelta\(seconds=(.*?[0-9]{0,5}).*content=(\'.*\'|\".*\"), proprietary',str(item))
                ...
                #content = srt_dic["content"].lstrip().rstrip()
